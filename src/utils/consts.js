...
const getTotal = (items) => {
  return items
    .map((item) => item.total)
    .reduce((sum, num) => (parseFloat(sum) + parseFloat(num)).toFixed(2));
};
...
export const getError = (splitted, errors, touched) => {
  if (
    ...
    touched[splitted[0]] &&
    errors[splitted[0]] &&
    touched[splitted[0]][splitted[1]] &&
    errors[splitted[0]][splitted[1]] &&
    touched[splitted[0]][splitted[1]][splitted[3]] &&
    errors[splitted[0]][splitted[1]][splitted[3]]
  ) {
    return true;
  }
  if (
...
export const createInvoiceId = () => {
  return `${alphabet[getRandomNumber(0, alphabet.length - 1)]}${
    alphabet[getRandomNumber(0, alphabet.length)]
