...
export const FooterNewInvoice = () => {
    ...
    return (
        ...
        dark:bg-dark-300 fixed md:px-[56px] bottom-0 left-0 pb-4 lg:static lg:ml-0 lg:pl-0 lg:pr-0 lg:mt-0 lg:mb-3">
            ...
            <Button styles="bg-dark-500  text-neutral-300 dark:bg-dark-200 dark:text-neutral-200 md:hidden"
                    type="button"
                    title="Save as Draft"
                    onClick={handleSavingAsDraft} />
            <Button styles="bg-primary-200 text-white md:hidden"
                    type="submit"
                    title="Save & Send"
                    onClick={() => setFieldValue('status', 'pending')} />
            <div className="hidden md:flex md:space-x-2 lg:space-x-[6px]">
                <Button styles="bg-dark-500  text-neutral-300 dark:bg-dark-200 dark:text-neutral-200"
                        type="button" title="Save as Draft"
                        onClick={handleSavingAsDraft} />
                <Button styles="bg-primary-200 text-white"
                        type="submit"
                        title="Save & Send"
                        onClick={() => setFieldValue('status', 'pending')} />
            </div>
