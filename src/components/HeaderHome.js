...
export const HeaderHome = () => {
  ...
  let invoiceNumberInfoVisibility =
    'hidden md:inline text-base/2 text-neutral-300 md:col-span2';
  if (invoicesNumber === 0) {
    invoicesNumberInfo = (
      <p className={invoiceNumberInfoVisibility}>No invoices</p>
    );
  } else if (invoicesNumber === 1) {
    invoicesNumberInfo = (
      <p className={invoiceNumberInfoVisibility}>There is 1 invoice</p>
    );
  } else {
    invoicesNumberInfo = (
      <p className={invoiceNumberInfoVisibility}>
        There are {invoicesNumber} total invoices
      </p>
    );
  }
  ...
  return (
    <div className="grid grid-cols-3 md:grid-cols-4 grid-rows-2">
      <h1 className="text-lg dark:text-light-100 md:text-xl md:col-span-2">
        Invoices
      </h1>
      <button className="relative row-span-2 flex justify-end items-center">
        <div
          className="text-dark-300 dark:text-light-100 text-md/2 mr-3"
