...
export const AlertModal = (onClose, onDelete) => {
	return (
		<div className='absolute bg-black opacity-50 w-screen h-screen top-0 left-0'>
		<div role='dialog' id='alert-modal' className='flex flex-col gap-3 absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 max-w-[30rem] shadow-3xl bg-light-100 dark:bg-dark-200 p-4 md:p-12 rounded-lg z-20'>
			<p className="text-md/1 lg:text-lg dark:text-light-100 font-bold">Confirm deletion</p>
			...
			<form className='flex gap-2 self-end'>
				<Button onClick={onClose} styles="bg-neutral-100 text-neutral-500" title="Cancel" type="button" />
